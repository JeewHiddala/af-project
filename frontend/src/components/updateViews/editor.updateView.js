...
class updateEditor extends Component {
    ...
    componentDidMount() {
        ...
        const editor = this.props.match.params.id;
        console.log("rrrr"+editor);
        axios.get(`http://localhost:7000/editor/${editor}`)
           .then(response => {
               ...
               this.setState({editorSalary:response.data.data.editorSalary})
    ...
    onSubmit(e) {      //submit details
        ...
        axios.patch(`http://localhost:7000/editor/update/${this.state.id}`, editor)
            .then(response => {
